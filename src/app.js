...
function renderButtons() {
  const ingredients = ["Cheese","Onions","Tomatoes", "Lettuce", "Patty"];
 
  ingredients.forEach((ingredient) => {
     const btnClass = `.btn-${ingredient.toLowerCase()}`;
...
function renderIngredientsBoard() {
  const ingredientItems = document.querySelectorAll(".items");
 
  ingredientItems.forEach((element) => {
     const ingredientName = element.textContent.trim();
     const isIngredientActive = state[ingredientName];
 
     element.style.display = isIngredientActive ? "block" : "none";
  });
 }
...
function renderPrice() {
 ...
 for (let ingredient in state) {
  ...
  document.querySelector(selectedButton).onclick = handleIngredientClick(ingredient);
 }
